...
function applyImportResults(results: ImportResult[]) {
  ...
  const topCategory = sortedCategories[0]?.[0] || (allTransactions.length > 0 ? "Import" : "");
  ...
  const recommendation =
    expenses > income
      ...
      : topCategoryAmount > 0
      ? `Behalte besonders "${topCategory}" im Blick. Dort liegen aktuell ${topCategoryAmount.toLocaleString("de-DE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€.`
...
• Auffällige Kategorie: ${topCategory} (${topCategoryAmount.toLocaleString("de-DE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€)
• Größte Ausgabe: ${biggestExpense?.name || "Keine"} (${biggestExpense ? Math.abs(biggestExpense.amount).toLocaleString("de-DE", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "0,00"}€)
• Sparscore: ${score}/100
          ...
          <div className="space-y-1 mt-2">
            ...
            <div className={financeSection === "upload" ? "block pt-6" : "hidden"}>
              <Panel isLightMode={isLightMode} title="Kontoauszüge & Uploads">
                ...
                {analysisResult && (
                  <div className={isLightMode ? "mt-3 bg-white border border-gray-200 rounded-2xl p-5 whitespace-pre-line text-black shadow-sm" : "mt-3 bg-white/10 border border-white/10 rounded-2xl p-5 whitespace-pre-line text-white"}>
                    {analysisResult}
                  </div>
                )}
              </Panel>
