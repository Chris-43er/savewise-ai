...
const [foundationStatus, setFoundationStatus] = useState("Analyse läuft");
...
useEffect(() => {
  ...
  if (monthlyIncome > 0 && totalMonthlyExpenses > 0) {
    ...
    if (savingRate >= 40) {
      generatedInsights.push({
        id: "great-saving",
        type: "opportunity",
        title: "Sehr starke Sparquote",
        description:
          "Du sparst aktuell deutlich über dem Durchschnitt.",
        priority: 1
      });
    }
    ...
    if (savingRate < 15) {
      generatedInsights.push({
        id: "low-saving",
        type: "warning",
        title: "Niedrige Sparquote",
        description:
          "Deine monatlichen Rücklagen sind aktuell niedrig.",
        priority: 2
      });
    }

    if (totalMonthlyExpenses > monthlyIncome) {
      generatedInsights.push({
    ...
    if (streamingCosts >= 40) {
      generatedInsights.push({
        id: "streaming-warning",
        type: "info",
        title: "Streamingkosten prüfen",
        description:
          "Mehrere Streamingdienste erkannt.",
        priority: 4
      });
    }
    ...
    const fixedRatio =
      monthlyIncome > 0
        ? Math.round((contractMonthlyTotal / monthlyIncome) * 100)
        : 0;
    ...
    if (fixedRatio > 55) {
      generatedInsights.push({
        ...
        description:
          "Deine laufenden Verpflichtungen sind relativ hoch.",
        priority: 3
      });
    }
  }
  ...
  if (generatedInsights.length > 0) {
    setFoundationStatus(generatedInsights[0].title);
  } else {
    setFoundationStatus("Finanzlage stabil");
  }
...
/* =========================================================
   DYNAMIC HOME PRIORITY ENGINE
========================================================= */

useEffect(() => {

  if (totalMonthlyExpenses > monthlyIncome && monthlyIncome > 0) {
    setDynamicPriority("warning");
    return;
  }

  if (contractMonthlyTotal > monthlyIncome * 0.5) {
    setDynamicPriority("contracts");
    return;
  }

  if (aiInsights.length > 0) {
    setDynamicPriority("ai");
    return;
  }

  setDynamicPriority("overview");
...
useEffect(() => {
  ...
  if (monthlyIncome > 0 && totalMonthlyExpenses > monthlyIncome) {
    notes.push("Ausgaben über Einkommen");
  }

  if (contractMonthlyTotal > 0) {
    notes.push(
      contractExpenses.length +
        " aktive Verträge erkannt"
    );
  }

  if (savingScore >= 80) {
    notes.push("Sehr gute Finanzstruktur");
  }

  setSmartNotifications(notes);
...
${smartTip}`;
  ...
  function resetAllData() {
    Object.keys(localStorage).forEach((key) => {
      const keepKeys = ["savewise_intro_seen", "savewise_onboarding_done"];
