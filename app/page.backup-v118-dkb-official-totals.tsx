...
async function parsePdfFile(file: File): Promise<ImportResult> {
  ...
  function shouldIgnoreDescription(text: string) {
    const v = text.toLowerCase();

    return (
      v.includes("kontostand am") ||
      v.includes("gesamtumsatzsummen") ||
      v.includes("summe soll") ||
      v.includes("summe haben") ||
      v.includes("dispositionskredit") ||
      v.includes("anzahl anlagen") ||
      v.includes("entgeltinformation") ||
      v.includes("abrechnungsinformation") ||
      v.includes("deutsche kreditbank") ||
      v.includes("taubenstraße") ||
      v.includes("telefon 030") ||
      v.includes("handelsregister") ||
      v.includes("seite ") ||
      v.includes("betrag soll") ||
      v.includes("betrag haben")
    );
  }
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    if (pageText.includes("Entgeltinformation: Anlage")) {
      continue;
    }
    ...
    for (const item of content.items as any[]) {
      ...
      const isEuroAmount = /^[+\-−]?\d{1,3}(?:\.\d{3})*,\d{2}$/.test(raw);

      if (isEuroAmount) {
        const cleanRaw = raw.replace("−", "-");
        const value = euro(cleanRaw);

        if (value === 0 || value > 20000) continue;
        if (shouldIgnoreDescription(lastDescription)) {
          lastDescription = "";
          continue;
        }

        const cleanName =
          lastDescription && lastDescription.length > 2
            ? lastDescription
            : cleanRaw.startsWith("-")
            ? "PDF Ausgabe"
            : "PDF Einnahme";
      ...
      if (
        ...
        raw.match(/^\d{1,2}\.\s?[a-zäöü]+\.?\s?\d{4}$/i) ||
        shouldIgnoreDescription(raw)
      ) {
        continue;
      }

      lastDescription = raw.length > 100 ? raw.slice(0, 100) : raw;
    }
  }
