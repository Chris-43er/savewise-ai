...
${smartTip}`;
  ...
  const contractInsight =
    contractExpenses.length === 0
      ...
      : contractRatio >= 50
      ? `Deine Fixkosten liegen bei ${contractRatio}% deines Einkommens. Das ist kritisch hoch. Prüfe zuerst die größten Positionen wie Wohnen, Versicherungen und Verträge.`
      : contractRatio >= 35
      ? `Deine Fixkosten liegen bei ${contractRatio}% deines Einkommens. Das ist erhöht. Einzelne Verträge könnten optimierbar sein.`
      : contractRatio >= 20
      ? `Deine Fixkosten liegen bei ${contractRatio}% deines Einkommens. Das wirkt solide, sollte aber regelmäßig geprüft werden.`
      : `Nur ${contractRatio}% deines Einkommens entfallen auf Fixkosten. Deine finanzielle Flexibilität ist aktuell sehr hoch.`;
  ...
  const topExpenseItems = [...allExpenseItems]
    .filter((item) => Math.abs(item.amount) >= 25)
    .sort((a, b) => Math.abs(b.amount) - Math.abs(a.amount))
    .slice(0, 2);

  const contractGroups = Object.values(
    contractExpenses.reduce((acc: Record<string, { category: string; total: number; items: typeof contractExpenses }>, item) => {
      const category = item.category || "Sonstiges";
      const interval = item.interval || "monatlich";
      const monthlyAmount =
        interval === "jährlich" ? item.amount / 12 :
        interval === "vierteljährlich" ? item.amount / 3 :
        item.amount;

      if (!acc[category]) acc[category] = { category, total: 0, items: [] };
...
<Panel isLightMode={isLightMode} title="Verträge & Fixkosten">
                  ...
                  {contractGroups.length > 0 && (
                    <div className="grid gap-2 mt-2">
                      <p className={isLightMode ? "text-xs font-black uppercase tracking-[0.18em] text-black/45" : "text-xs font-black uppercase tracking-[0.18em] text-white/40"}>
                        Vertragsgruppen
                      </p>
                      {contractGroups.map((group) => (
                        <div key={group.category} className={isLightMode ? "rounded-2xl bg-white border border-gray-200 p-4 shadow-sm" : "rounded-2xl bg-white/[0.055] border border-white/10 p-4"}>
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <p className={isLightMode ? "font-black text-black" : "font-black text-white"}>
                                {group.category}
                              </p>
                              <p className={isLightMode ? "text-xs text-black/55" : "text-xs text-white/50"}>
                                {group.items.length} Vertrag{group.items.length === 1 ? "" : "e"}
                              </p>
                            </div>
                            <p className="font-black text-emerald-400">
                              {group.total.toLocaleString("de-DE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€
                            </p>
                          </div>
                        </div>
                      ))}
            ...
            {homeSection === "overview" && topExpenseItems.length > 0 && (
              <div className={isLightMode ? "rounded-[22px] border border-gray-200 bg-white px-4 py-2.5 shadow-lg shadow-black/5" : "rounded-[22px] border border-white/10 bg-white/[0.045] px-4 py-2.5 shadow-lg shadow-black/20"}>
                <p className={isLightMode ? "text-xs font-black uppercase tracking-[0.18em] text-black/60" : "text-xs font-black uppercase tracking-[0.18em] text-white/45"}>
                  Größte Belastungen
                </p>
                ...
                <div className="mt-1.5 grid gap-1.5">
                  {topExpenseItems.map((item, index) => (
                    <div key={index} className="flex items-center justify-between gap-3">
                      <p className={isLightMode ? "font-black text-black truncate" : "font-black text-white truncate"}>
                        {index === 0 ? "🏠" : index === 1 ? "💳" : "📌"} {item.name}
                      </p>
                      <p className="text-sm font-black text-red-400 whitespace-nowrap">
                        {Math.abs(item.amount).toLocaleString("de-DE", { maximumFractionDigits: 0 })}€
                      </p>
                    </div>
                  ))}
                </div>
...
<div id="home-menu" className="grid gap-2 -mt-1 pb-24">
  {[
    {
      key: "compare",
