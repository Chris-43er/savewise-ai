...
async function parseImageFile(file: File): Promise<ImportResult> {
  ...
  return {
    ...
    expenses,
    transactions: [
      ...(income > 0 ? [{ name: "Bild Einnahmen", amount: income, category: "Einkommen" }] : []),
      ...(expenses > 0 ? [{ name: "Bild Ausgaben", amount: -expenses, category: "Ausgaben" }] : [])
    ],
    source: file.name
  };
}
                ...
                <label className="mt-2 flex items-center gap-1 bg-emerald-400 text-black px-6 py-4 rounded-2xl font-black cursor-pointer w-fit">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      await analyzeFilesTogether(files);
