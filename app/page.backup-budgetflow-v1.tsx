        ...
        {activeTab === "home" && (
          <div className="space-y-6 mt-6">
            ...
            <div className="relative rounded-[42px] border border-emerald-400/20 bg-gradient-to-br from-emerald-400/10 via-cyan-400/5 to-transparent p-4 shadow-[0_0_55px_rgba(16,185,129,0.16)]">
              ...
              <div className="mb-6 rounded-[42px] border border-white/10 bg-white/[0.035] p-5 shadow-2xl backdrop-blur-xl overflow-hidden">
                <div className="mb-5">
                  <p className="text-xs uppercase tracking-[0.24em] text-emerald-300 font-black">
                    SaveWise Orbit
                  </p>
                  <p className="text-white/50 text-sm mt-1">
                    Deine Finanzbereiche als Ring-Navigation
                  </p>
                </div>

                <div className="relative mx-auto w-full max-w-[320px] h-[370px]">

                  <svg viewBox="0 0 290 290" className="absolute left-1/2 top-[54px] w-[290px] h-[290px] -translate-x-1/2">
                    <circle cx="145" cy="145" r="96" fill="none" stroke="rgba(255,255,255,0.07)" strokeWidth="30" />

                    <path d="M 91 66 A 96 96 0 0 1 199 66" fill="none" stroke="rgba(52,211,153,0.82)" strokeWidth="30" strokeLinecap="round"
                      onClick={() => { setActiveTab("finance"); setFinanceSection("manual"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                      className="cursor-pointer" />

                    <path d="M 224 91 A 96 96 0 0 1 224 199" fill="none" stroke="rgba(248,113,113,0.82)" strokeWidth="30" strokeLinecap="round"
                      onClick={() => { setActiveTab("finance"); setFinanceSection("transactions"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                      className="cursor-pointer" />

                    <path d="M 199 224 A 96 96 0 0 1 91 224" fill="none" stroke="rgba(192,132,252,0.82)" strokeWidth="30" strokeLinecap="round"
                      onClick={() => setShowScoreInfo(true)}
                      className="cursor-pointer" />

                    <path d="M 66 199 A 96 96 0 0 1 66 91" fill="none" stroke="rgba(250,204,21,0.86)" strokeWidth="30" strokeLinecap="round"
                      onClick={() => { setHomeSection("monthly"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                      className="cursor-pointer" />
                  </svg>
                  ...
                  <button
                    ...
                    className="absolute left-1/2 top-[199px] w-32 h-32 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#050816] border border-emerald-400/25 shadow-[0_0_45px_rgba(16,185,129,0.28)] flex flex-col items-center justify-center text-center active:scale-95 transition-all z-20"
                  >
                    <span className="text-2xl">🤖</span>
                    <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.18em] mt-1">
                      Verfügbar
                    </p>
                    <p className="text-yellow-400 text-2xl font-black mt-1">
                      {monthlyIncome > 0 ? remainingAfterManual.toLocaleString("de-DE", { maximumFractionDigits: 0 }) + "€" : "—"}
                    </p>
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => { setActiveTab("finance"); setFinanceSection("manual"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                    className="absolute left-1/2 top-0 -translate-x-1/2 rounded-2xl bg-emerald-400/10 border border-emerald-400/20 px-4 py-2 text-center active:scale-95 transition-all z-30"
                  >
                    <p className="text-[9px] uppercase tracking-[0.16em] text-emerald-300 font-black">Einkommen</p>
                    <p className="text-emerald-400 text-sm font-black">{monthlyIncome > 0 ? monthlyIncome.toLocaleString("de-DE", { maximumFractionDigits: 0 }) + "€" : "—"}</p>
                  </button>

                  <button
                    type="button"
                    onClick={() => { setActiveTab("finance"); setFinanceSection("transactions"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                    className="absolute right-0 top-[174px] -translate-y-1/2 rounded-2xl bg-red-400/10 border border-red-400/20 px-3 py-2 text-center active:scale-95 transition-all z-30"
                  >
                    <p className="text-[9px] uppercase tracking-[0.16em] text-red-300 font-black">Ausgaben</p>
                    <p className="text-red-400 text-sm font-black">{totalMonthlyExpenses > 0 ? totalMonthlyExpenses.toLocaleString("de-DE", { maximumFractionDigits: 0 }) + "€" : "—"}</p>
                  </button>

                  <button
                    type="button"
                    onClick={() => { setHomeSection("monthly"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                    className="absolute left-0 top-[174px] -translate-y-1/2 rounded-2xl bg-yellow-400/10 border border-yellow-400/20 px-3 py-2 text-center active:scale-95 transition-all z-30"
                  >
                    <p className="text-[9px] uppercase tracking-[0.16em] text-yellow-300 font-black">Rest</p>
                    <p className="text-yellow-400 text-sm font-black">{monthlyIncome > 0 ? remainingAfterManual.toLocaleString("de-DE", { maximumFractionDigits: 0 }) + "€" : "—"}</p>
                  </button>

                  <button
                    type="button"
                    onClick={() => setShowScoreInfo(true)}
                    className="absolute left-1/2 bottom-0 -translate-x-1/2 rounded-2xl bg-purple-400/10 border border-purple-400/20 px-4 py-2 text-center active:scale-95 transition-all z-30"
                  >
                    <p className="text-[9px] uppercase tracking-[0.16em] text-purple-300 font-black">Score</p>
                    <p className="text-purple-400 text-sm font-black">{savingsRate}%</p>
                  </button>
...
<Panel isLightMode={isLightMode} title="Sparziel">
  ...
  type="button"
  onClick={() => setIsAiHubOpen((open) => !open)}
  className="fixed right-5 bottom-28 z-[9999] w-14 h-14 rounded-3xl bg-emerald-400/20 border border-emerald-400/30 backdrop-blur-xl flex items-center justify-center"
  ...
  return (
    <div className="relative mb-4 flex items-center justify-start">
      <div className="relative">
        <div className="absolute inset-0 rounded-[18px] bg-emerald-400/25 blur-lg" />

        <img
          src="/savewise-icon.png"
          alt="SaveWise Logo"
          className="relative w-14 h-14 rounded-[16px] object-cover shadow-xl shadow-emerald-400/20"
        />
      </div>
    </div>
  );
...
function Mini(props: { title: string; value: string; color: string }) {
  return (
    <div className="bg-gray-100 rounded-2xl p-4 flex flex-col items-center justify-center text-center min-h-[118px] overflow-hidden">
      ...
      <p className={`text-lg font-black mt-1 ${props.color}`}>{props.value}</p>
    </div>
  );
}
