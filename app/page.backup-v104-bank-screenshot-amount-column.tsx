...
async function parseImageFile(file: File): Promise<ImportResult> {
  ...
  const strictAmountRegex = /([+-]\s*\d{1,3}(?:[.\s]\d{3})*,\d{2})\s*€/g;

  const pushAmount = (rawValue: string, sourceLine: string) => {
    ...
    const isIncome = cleaned.startsWith("+");
    const cleanName =
      sourceLine
        .replace(strictAmountRegex, "")
        .replace(/\s+/g, " ")
        .trim()
        .slice(0, 80) || (isIncome ? "Bild Einnahme" : "Bild Ausgabe");
  ...
  for (const line of lines) {
    const matches = Array.from(line.matchAll(strictAmountRegex));
    matches.forEach((match) => pushAmount(match[1], line));
  }

  // Fallback: Manche OCRs trennen Betrag und Eurozeichen in verschiedene Fragmente.
  // Dann wird der gesamte Text nochmals geprüft, aber weiterhin nur mit eindeutigem + oder -.
  if (transactionsFromImage.length === 0) {
    const matches = Array.from(normalizedText.matchAll(strictAmountRegex));
    matches.forEach((match) => pushAmount(match[1], "Bild Buchung"));
  }
