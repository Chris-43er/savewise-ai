        ...
        {activeTab === "home" && (
          <div className="space-y-5 mt-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              />
              <Card title="Sparquote" value={transactions.length > 0 && monthlyIncome > 0 ? Math.max(0, Math.round(((monthlyIncome - spentThisMonth) / monthlyIncome) * 100)) + "%" : "—"} color="text-purple-400" />
            </div>
            )}
...
<div id="home-menu" className="grid gap-4">
  {[
    ...
    <button
      ...
      className={
        "relative overflow-hidden rounded-[28px] border p-4 text-left transition-all active:scale-[0.98] shadow-xl " +
        (homeSection === item.key
          ? "bg-emerald-400 text-black border-emerald-300 shadow-2xl shadow-emerald-400/25"
          : "bg-white/5 text-white border-white/10 shadow-black/20")
      }
    >
              ...
              <div className="grid gap-4">
                {[
                  ...
                  >
                    ...
                    <p className={homeSection === item.key ? "mt-1 text-white" : "mt-1 text-gray-300"}>{item.text}</p>
                  </button>
                ))}
              </div>
            )}
...
<div className="mt-6 w-full max-w-3xl rounded-[30px] border border-white/10 bg-black/20 p-4 backdrop-blur-xl text-white">
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-[22px] border border-emerald-400/20 bg-emerald-400/10 px-3 py-5 text-center">
              <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-emerald-300">
                Sparen
              </p>
              <p className="mt-2 text-lg font-black text-white whitespace-nowrap">
                {props.monthlySavings}€
              </p>
            </div>

            <div className="rounded-[22px] border border-cyan-400/20 bg-cyan-400/10 px-3 py-5 text-center">
            ...
            <div className="rounded-[22px] border border-purple-400/20 bg-purple-400/10 px-3 py-5 text-center">
              <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-purple-300">
                Kategorie
              </p>
              <p className="mt-2 text-sm font-black text-white whitespace-nowrap">
                {props.topCategory}
              </p>
            </div>
          </div>
...
function Card(props: { title: string; value: string; color: string; note?: string }) {
  const isAI = props.title === "KI Insight";

  return (
    <div
      className={
        "relative overflow-hidden min-h-[118px] rounded-[30px] border p-5 backdrop-blur-2xl shadow-2xl transition-all active:scale-[0.98] " +
        (isAI
          ? "border-cyan-400/30 bg-cyan-400/10 shadow-cyan-400/10"
...
function Panel(props: {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-[30px] p-7 backdrop-blur-2xl shadow-xl shadow-black/20">
      <h2
        className={
