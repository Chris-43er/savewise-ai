...
const [savingGoal, setSavingGoal] = useState("Notgroschen");
...
<div className="grid gap-3 mt-6">
  ...
  " border border-white/10 rounded-2xl px-4 py-3 outline-none"
}
  />
  ...
  <p className="text-emerald-400 font-black text-3xl mt-4">
    Noch nötig: {goalAmount - savedAmount}€
  </p>

  <div className="w-full h-5 bg-black/30 rounded-full mt-6 overflow-hidden">
    <div
      className="h-full bg-cyan-400 rounded-full"
      style={{
        width: Math.min(100, (savedAmount / goalAmount) * 100) + "%"
      }}
    />
  </div>
</Panel>
            <Panel title="Monatsbudget">
              <div className="flex gap-3 mt-6">
                <input
                  ...
                  placeholder="Budget"
                  className="bg-black/30 border border-white/10 rounded-2xl px-4 py-3 w-full outline-none"
                />
        ...
        {activeTab === "analyse" && (
          <div className="space-y-8 mt-8">
            ...
            <Panel title="Analyse Verlauf">
              <div className="space-y-4 mt-6">
                {history.length === 0 && <p className="text-gray-500">Noch keine Analysen vorhanden.</p>}
                {history.map((item, index) => (
                  <div key={index} className="bg-black/30 p-4 rounded-2xl text-gray-300">
                    {item}
                  </div>
                ))}
              </div>
            ...
            <Panel title="Letzte Transaktionen">
              <div className="space-y-4 mt-6">
                {transactions.map((item, index) => (
                  <div key={index} className="bg-black/30 p-5 rounded-2xl flex justify-between items-center">
                    <div>
        ...
        {activeTab === "report" && (
          <div className="space-y-8 mt-8">
            <Panel title="Datei Upload">
              ...
              {analysisResult && (
                <div className="mt-5 bg-black/30 border border-cyan-400/30 rounded-2xl p-5 text-white font-bold whitespace-pre-line">
                  {analysisResult}
                </div>
              )}
            </Panel>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xl z-50 flex items-end md:items-center justify-center p-6">
          <div className="w-full max-w-lg bg-[#0B1120] border border-white/10 rounded-[32px] p-8 shadow-2xl">
...
<button
  ...
  className="w-full bg-white/10 text-white rounded-2xl p-4 font-black"
>
  {isLightMode ? "Dark Mode aktivieren" : "Light Mode aktivieren"}
</button>

<div className="bg-black/30 rounded-2xl p-5 text-gray-300">
  SaveWise AI MVP · Lokale Demo-Version
      ...
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 bg-black/80 border border-white/10 backdrop-blur-2xl rounded-full px-8 py-4 flex gap-8 shadow-2xl z-50">
...
function Header() {
  return (
    <div className="bg-white/5 border border-white/10 rounded-[32px] p-8 backdrop-blur-2xl shadow-2xl shadow-emerald-500/10">
...
function Card(props: { title: string; value: string; color: string }) {
  return (
    <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-2xl shadow-2xl shadow-black/30">
...
function Panel(props: {
  ...
  return (
    <div
  className={
    "rounded-3xl p-8 backdrop-blur-2xl shadow-2xl border " +
    (isLightMode
      ? "bg-white border-gray-200 text-gray-900 shadow-gray-200"
      : "bg-white/5 border-white/10 text-white shadow-black/30")
  }
>
      <h2 className="text-4xl font-black">{props.title}</h2>
      {props.children}
